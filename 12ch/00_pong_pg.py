...
if __name__ == "__main__":
    ...
    with common.RewardTracker(writer, stop_reward=18) as tracker:
        for step_idx, exp in enumerate(exp_source):
            ...
            writer.add_scalar("kl", kl_div_v.data.cpu().numpy(), step_idx)

            writer.add_scalar("baseline", baseline, step_idx)
            writer.add_scalar("entropy", entropy_v.data.cpu().numpy(), step_idx)
            writer.add_scalar("batch_scales", np.mean(batch_scales), step_idx)
            writer.add_scalar("batch_scales_std", scale_std, step_idx)
            writer.add_scalar("loss_entropy", entropy_loss_v.data.cpu().numpy(), step_idx)
            writer.add_scalar("loss_policy", loss_policy_v.data.cpu().numpy(), step_idx)
            writer.add_scalar("loss_total", loss_v.data.cpu().numpy(), step_idx)
